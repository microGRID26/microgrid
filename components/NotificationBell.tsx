...
import { Bell, AlertTriangle, OctagonX, CheckCheck, AtSign, Settings } from 'lucide-react'
import { useNotifications, type Notification } from '@/lib/useNotifications'
import { usePreferences, type NotificationPrefs, DEFAULT_NOTIFICATION_PREFS } from '@/lib/usePreferences'
...
export function NotificationBell() {
  ...
  const togglePref = (key: keyof NotificationPrefs) => {
    ...
    updatePref('notification_prefs', next)
  }
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div role="menu" aria-label="Notifications" className="absolute right-0 top-full mt-1 w-80 bg-gray-900 border border-gray-700 rounded-lg shadow-xl z-[100] overflow-hidden">
          {/* Header */}
          <div className="flex items-center justify-between px-3 py-2 border-b border-gray-700">
            <span className="text-sm font-medium text-white">{showSettings ? 'Notification Settings' : 'Notifications'}</span>
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`text-gray-400 hover:text-white p-0.5 transition-colors ${showSettings ? 'text-green-400' : ''}`}
          ...
          {/* Settings panel */}
          {showSettings && (
            <div className="px-3 py-3 border-b border-gray-700 space-y-2.5">
              {([
                { key: 'blocked' as const, label: 'Blocked project alerts', desc: 'In-app alerts when projects are blocked' },
                { key: 'stuck_tasks' as const, label: 'Stuck task alerts', desc: 'In-app alerts for Pending Resolution / Revision Required' },
                { key: 'mentions' as const, label: '@Mention notifications', desc: 'When someone mentions you in a note' },
                { key: 'stuck_email' as const, label: 'Email on stuck tasks', desc: 'Send email when a task enters Pending Resolution' },
                { key: 'digest_email' as const, label: 'Morning digest email', desc: 'Daily briefing with your portfolio summary' },
              ]).map(item => (
                <label key={item.key} className="flex items-start gap-2.5 cursor-pointer group">
                  <input
                    ...
                    className="mt-0.5 rounded border-gray-600 bg-gray-800 text-green-500 focus:ring-green-500 focus:ring-offset-0"
                  />
                  <div>
                    <div className="text-xs text-white group-hover:text-green-300 transition-colors">{item.label}</div>
