...
export async function loadInvoices(orgId?: string, status?: InvoiceStatus): Promise<Invoice[]> {
  ...
  let q = supabase
    .from('invoices')
    .select('id, invoice_number, project_id, from_org, to_org, status, milestone, subtotal, tax, total, due_date, sent_at, viewed_at, paid_at, paid_amount, payment_method, payment_reference, notes, generated_by, snapshot_id, rule_id, created_by, created_by_id, created_at, updated_at, rule:invoice_rules(name)')
...
export async function loadInvoice(invoiceId: string): Promise<{ invoice: Invoice; lineItems: InvoiceLineItem[] } | null> {
  ...
  const [invResult, itemsResult] = await Promise.all([
    supabase.from('invoices').select('id, invoice_number, project_id, from_org, to_org, status, milestone, subtotal, tax, total, due_date, sent_at, viewed_at, paid_at, paid_amount, payment_method, payment_reference, notes, generated_by, snapshot_id, rule_id, created_by, created_by_id, created_at, updated_at, rule:invoice_rules(name)').eq('id', invoiceId).single(),
